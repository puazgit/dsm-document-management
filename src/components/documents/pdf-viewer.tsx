...
export function SecurePDFViewer({ 
  ...
  useEffect(() => {
    const loadPermissions = async () => {
      ...
      try {
        ...
        // If we have session permissions, use them
        if (userPermissions.length > 0) {
          const canDownload = userPermissions.includes('pdf.download.all') || 
                            userPermissions.includes('documents.download.all');
          const canPrint = userPermissions.includes('pdf.print.all');
          const canCopy = userPermissions.includes('pdf.copy.all');

          console.log('🔍 Using Database Permissions:', {
            userRole,
            userPermissions,
            canDownload,
            canPrint,
          ...
          // Fallback to hardcoded permissions (for demo/testing)
          console.log('🔄 Using Fallback Permissions for role:', userRole);
          const fallbackPermissions = getFallbackPermissions(userRole);
          setCurrentPermissions(fallbackPermissions);
        }
      } catch (error) {
        console.error('Error loading permissions:', error);
        // Fallback to hardcoded permissions for backwards compatibility
        const fallbackPermissions = getFallbackPermissions(userRole);
        setCurrentPermissions(fallbackPermissions);
      } finally {
        setPermissionsLoaded(true);
  ...
  const getFallbackPermissions = (role: string) => {
    const rolePermissions = {
      ...
      'org_komite_audit': { canDownload: true, canPrint: true, canCopy: false },
      'org_gm': { canDownload: true, canPrint: true, canCopy: false },
      'org_kadiv': { canDownload: true, canPrint: true, canCopy: false },
      'org_manager': { canDownload: true, canPrint: true, canCopy: false },
      'org_finance': { canDownload: true, canPrint: false, canCopy: false },
      'org_hrd': { canDownload: true, canPrint: false, canCopy: false },
      'org_supervisor': { canDownload: true, canPrint: false, canCopy: false },
      'org_sekretaris': { canDownload: false, canPrint: false, canCopy: false },
      'org_staff': { canDownload: false, canPrint: false, canCopy: false },
      'org_guest': { canDownload: false, canPrint: false, canCopy: false }
    };
  ...
  // Debug final result
  console.log('🎯 Final Download Permission:', {
    canDownload,
    'currentPermissions.canDownload': currentPermissions.canDownload,
    canUserDownload,
    permissionsLoaded
  });
  ...
  if (!permissionsLoaded) {
    return (
      <div className="w-full bg-white border rounded-lg shadow">
        <div className="p-6">
          <div className="p-4 text-center text-blue-600 border border-blue-200 rounded-lg bg-blue-50">
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg bg-gray-50">
              <div className="flex items-center gap-4">
                {canUserDownload ? (
                  ...
                  <button
                    disabled
                    className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 border border-gray-300 rounded-md cursor-not-allowed"
...
// Default export and backward compatibility
export default SecurePDFViewer;
export const PDFViewer = SecurePDFViewer;